			...
			for (int j = 1; j <= v[i].length(); j++) {
				for (int k = n - 1; k >= 0; k--) {
					...
					if (!pref[i][j - v[k].length()])
						continue;
					if (voc.contains(v[i].substring(j - v[k].length(), j))) {
						pref[i][j] = true;
						break;
					}
				}
			...
			for (int j = v[i].length() - 1; j >= 0; j--) {
				for (int k = n - 1; k >= 0; k--) {
					...
					if (!suf[i][j + v[k].length()])
						continue;
					if (voc.contains(v[i].substring(j, j + v[k].length()))) {
						suf[i][j] = true;
						break;
					}
				}
			...
			for (int j = 0; j < v[i].length(); j++) {
				...
				for (int k = 0; k < n; k++) {
					if (j + v[k].length() <= v[i].length())
						break;
					if (!v[k].substring(0, v[i].length() - j).equals(v[i].substring(j))) {
						continue;
					}
					if (checked.contains(v[i].substring(0, j) + v[k]))
						continue;
					checked.add(v[i].substring(0, j) + v[k]);
