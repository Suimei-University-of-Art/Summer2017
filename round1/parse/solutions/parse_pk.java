...
public class parse_pk {
	...
	public String solve(String[] v) throws IOException {
		int n = v.length;
		assert((1 <= n) && (n <= 250));
		HashSet<String> was = new HashSet<String>();
		for (int i = 0; i < n; i++) {
			assert((1 <= v[i].length()) && (v[i].length() <= 500));
			assert((!was.contains(v[i])));
			was.add(v[i]);
		}
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 1; j <= v[i].length(); j++) {
				...
				assert(('a' <= v[i].charAt(j - 1)) && (v[i].charAt(j - 1) <= 'z'));
			}
		}
			...
			for (int j = 0; j < v[i].length(); j++) {
				...
				for (int k = 0; k < n; k++) {
					...
					if (checked.contains(hash[i][j] * pow[v[k].length()] + hash[k][v[k].length()]))
						continue;
					checked.add(hash[i][j] * pow[v[k].length()] + hash[k][v[k].length()]);
		...
		if (ti == -1) {
			...
			return v[ti].substring(0, tj) + v[tk];
		}
	}
