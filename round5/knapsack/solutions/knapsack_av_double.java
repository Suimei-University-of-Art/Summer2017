...
public class knapsack_av_double {
	...
	public void solve() {
		...
		for (int n = 1; n <= MAX_N; n++) {
			...
			for (int IT = 0; IT < MAX_ITERS; IT++) {
				...
				double[] doubleDp = new double[MAX_W + 1];
				doubleDp[0] = dp[0] = 1;

				for (int w : weights) {
					for (int i = MAX_W; i >= w; i--) {
						dp[i] += dp[i - w];
						doubleDp[i] += doubleDp[i - w];
					}
				}

				Item[] items = new Item[MAX_W + 1];
				for (int i = 0; i <= MAX_W; i++) {
					items[i] = new Item(doubleDp[i], dp[i], i);
				}
				Arrays.sort(items);
				int ptr = 0;
				...
				for (int i = 0; i < MAX_N - n; i++) {
					while (ptr < items.length
							&& (items[ptr].dVal > 2 * MAX_VAL || items[ptr].lVal > cur)) {
						ptr++;
					}
	...
	class Item implements Comparable<Item> {
		...
		public Item(double dVal, long lVal, int weight) {
			this.dVal = dVal;
			this.lVal = lVal;
			this.weight = weight;
		}
		...
		public int compareTo(Item o) {
			return -Double.compare(dVal, o.dVal);
		}
