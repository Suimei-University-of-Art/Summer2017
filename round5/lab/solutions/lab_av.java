...
public class lab_av {
	...
	public void solve() {
		...
		for (int i = 0; i < k; i++) {
			...
			if (1l * C * q < p && p < 1l * H * q) {
				...
				num /= gcd;
				denom /= gcd;
				if (num <= MAX && denom <= MAX
						&& intersects((int) num, as[(int) num], (int) denom, bs[(int) denom])) {
					out.println("YES");
				} else {
					out.println("NO");
				}
	...
	int[][] table = new int[SQRT][SQRT];

	private boolean intersects(int p, BitSet a, int q, BitSet b) {
		if (p < SQRT && q < SQRT) {
			if (table[p][q] != 0) {
				return table[p][q] == 1;
			}
			
			boolean res = a.intersects(b);
			table[p][q] = res ? 1 : -1;
			return res;
		} else {
