...
public class timer_bm_biginteger {
	FastScanner in;
	PrintWriter out;
	...
	String gen(BigInteger value, Task t) {
		...
		while (tmp.length() < t.digits) {
			tmp = "0" + tmp;
		}
		char[] res = new char[t.digits];
		for (int i = 0; i < t.digits; i++) {
			res[i] = t.map[i][tmp.charAt(i) - '0'];
		}
	...
	BigInteger canDiff(char[] smaller, char[] bigger, Task t, int pos) {
		...
		BigInteger result = BigInteger.valueOf(10).pow(t.digits)
				.subtract(biggerBI);
		for (int p = pos; p >= 0; p--) {
			...
			BigInteger POW = BigInteger.valueOf(10).pow(t.digits - 1 - p);
