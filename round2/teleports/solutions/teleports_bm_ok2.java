...
public class teleports_bm_ok2 {
    ...
    void go2(int v) {
        for (; from[v] < roads[v].size(); from[v]++) {
            if (!roads[v].get(from[v]).used) {
				int tmp = from[v];
                roads[v].get(from[v]).used = true;
                roads[v].get(from[v]).back.used = true;
                if (pair[roads[v].get(from[v]).to] == -1)
                    go2(roads[v].get(from[v]).to); else 
                        go2(pair[roads[v].get(from[v]).to]);
                out.print(roads[v].get(tmp).id + " ");
            }
        }
    }
