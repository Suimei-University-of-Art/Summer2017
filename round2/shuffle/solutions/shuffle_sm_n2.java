...
//        //System.err.println("move " + l + " " + r + " in " + Arrays.toString(a));
    ...
    boolean solveIfBasic(int[] a, int length) {
        ...
        int l = -1;
        int r = -1;
        int[] problemCount = new int[a.length];
        ...
        int maxColor = -1;
        for (int color = 0; color < problemCount.length; color++) {
            if (problemCount[color] > (totalConflicts + 1) / 2) {
                maxColor = color;
        ...
        //System.err.println("problemCount[maxColor] = "  + problemCount[maxColor]);
        if (tooManyMaxColor) {
            int fatConflicts = problemCount[maxColor];
...
//                [l..r]
        ...
        if (tooManyMaxColor && l > 0 && wasC && length % 2 == 1 &&  problemCount[maxColor] + 1 == (sourceLength + 1) / 2 && remainVirtual > 0) {
...
//        //System.err.println(Arrays.toString(numberCount) + " " + ((n + 1) / 2));
        ...
        int[] conflictsColor = new int[a.length - 1];
        for (int i = 0; i < conflictsColor.length; i++) {
            conflictsColor[i] = -1;
            if (a[i] == a[i + 1]) {
                conflictsColor[i] = a[i];
            }
        }
        int length = a.length;
        while (true) {
